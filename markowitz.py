...
def plot_dailyreturns(returns):
	returns.plot(figsize=(10,5))
	plt.show()

#mean and covariance of stocks.252 trading days in a year
...
def calculate_portfolio_return(returns,weights):
	portfolio_returns=np.sum(returns.mean()*weights)*255
	print("Expected portfolio returns:",portfolio_returns)
	

def calculate_portfolio_variance(returns,weights):
	portfolio_variance=np.sqrt(np.dot(weights.T,np.dot(returns.cov()*255,weights)))
	print("Expected portfolio variance:",portfolio_variance)
	

#generate portfolios with monte carlo simulation
def generate_portfolios(weights,returns):
	preturns=[]
	pvariances=[]
	
	#monte carlo simulation:we generate several different werandom weights->random portfolios
	for i in range(10000):
		weights=calculate_weights()	
		preturns.append(np.sum(returns.mean()*weights)*255)	
		pvariances.append(np.sqrt(np.dot(weights.T,np.dot(returns.cov()*255,weights))))
...
show_data(data)
returns=calculate_returns(download_data(stocks))	
plot_dailyreturns(returns)
show_stats(returns)
weights=calculate_weights()
